...
export function createParticipantContainer(
    ...
    if (className) participant.classList.add(className);
...
export function createDateContainer(
    ...
    if (data) {
        ...
        datespan2.innerText = dateDate.time;
        datespan.innerText = dateDate.dayMonth;
    } else {
        date.classList.add("bracket-date-blank");
    }
    if (_id) {
        date.style.cursor = "pointer";
        date.addEventListener("click", () => {
            window.location.href = `/prime/tournament/match/${_id}`;
        });
    }
    date.append(datespan, datespan2);
    return date;
}
