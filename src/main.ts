...
export class BracketsViewer {
    ...
    private skipBracketMatch(): HTMLElement {
        ...
        const dateDiv = dom.createDateContainer(null, null);
        opponents.append(dateDiv);

        const participant1 = this.createParticipant("bracket-efgh", null, true);
        const participant2 = this.createParticipant("bracket-abcd", null, true);
    ...
    private createMatch(
        ...
        if (isMatch(match)) {
            ...
            const participant1 = this.createParticipant(
                match?.startTime ? null : "bracket-efgh",
                match.opponent1,
                propagateHighlight,
                "opponent1",
                originHint,
            ...
            let matchId =
                ...
                match.opponent2.id != null
                    ? match._id
                    : null;
            const dateDiv = dom.createDateContainer(
                match?.startTime || null,
                matchId || null
            );
            const dateDiv2 = dom.createDateContainer(null, null);
            opponents.append(dateDiv);
            opponents.append(participant1, participant2);
            opponents.append(dateDiv2);
        } else {
            const participant1 = this.createParticipant(
                match?.startTime ? null : "bracket-efgh",
                match.opponent1,
